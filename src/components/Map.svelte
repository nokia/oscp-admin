...
<script>
    ...
    function createMap(container) {
        ...
        m.on('zoomend', () => {
            ...
            updateMarker(marker, marker ? marker.getLatLng() : {lat: centerGeo, lng: centerGeo[1]},
                calcH3Resolution(), toolbarComponent);
        })
        ...
        toolbar.onAdd = () => {
            ...
            toolbarComponent.$on('select-geoposeservice', (event) =>
                updateGeoposeCoverageLayers(event.detail, coverageLayer));

            toolbarComponent.$on('checkcontent', (event) =>
                dispatch('goto-checkcontent', event.detail));
